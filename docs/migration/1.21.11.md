...
### `BlockStateModel`s are not initialized on Fabric

Fabric has not ported its model loading API yet. Mods using custom `BlockStateModel`s will run into `NullPointerException`s due to the models never initializing.
...
### Recipe Viewers Support has no effect currently

With the `ResourceLocation` to `Identifier` rename, we can't compile against an older API of JEI at the moment, so the implementation was temporarily stripped out until JEI updates.

Once JEI updates, a Balm update will re-add the inbuilt support.
